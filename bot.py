...
REPL_LOG_FILE = '/var/log/postgresql/postgresql-15-main.log'
...
def get_repl_logs(update: Update, context):
    try:
        ...
        update.message.reply_text(f'Текущий рабочий каталог: {current_dir}')
        update.message.reply_text(f'Путь к файлу логов: {REPL_LOG_FILE}')

        command = f"cat {REPL_LOG_FILE} | head -n 10"
        result = execute_ssh_command(SSH_HOST, SSH_PORT, SSH_USERNAME, SSH_PASSWORD, command)
    
        for line in result.splitlines():
                update.message.reply_text(line)
    except Exception as e:
        logger.error(f"Ошибка при чтении файла логов репликации: {str(e)}")
        update.message.reply_text('Произошла ошибка при чтении файла логов репликации.')
